...
import { Calendar, MapPin, Download, Send, X, Ticket as TicketIcon } from 'lucide-react';
...
export function Tickets() {
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.tickets(filters) });
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      {/* Cancel Dialog */}
      <Dialog open={!!cancelTicket} onOpenChange={() => setCancelTicket(null)}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCancelTicket(null)}>
              Keep Ticket
            </Button>
            <Button
              variant="destructive"
              onClick={() => cancelTicket && cancelMutation.mutate({
                id: cancelTicket.id,
      ...
      <Dialog open={!!transferTicket} onOpenChange={() => setTransferTicket(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setTransferTicket(null)}>
              Cancel
            </Button>
            <Button
              onClick={() => transferTicket && transferMutation.mutate({
